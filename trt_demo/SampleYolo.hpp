...
private:
    ...
    //!
    bool processInput_aspectRatio(const samplesCommon::BufferManager& buffers, cv::Mat &mSampleImage);
    ...
    //!
    bool verifyOutput_aspectRatio(const samplesCommon::BufferManager& buffers, cv::Mat& image, std::vector<BoundingBox>& nms_bboxes);
    ...
    bool infer_iteration(SampleUniquePtr<nvinfer1::IExecutionContext> &context, samplesCommon::BufferManager &buffers, cv::Mat &image, cv::Mat &outputFrame, int count);

    std::vector<BoundingBox> get_bboxes(int batch_size, int keep_topk, float* output);
